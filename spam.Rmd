...
for (i in plot.feats) {
  ...
  density_estimate_0 <- density(data[data$spam_target == 0, i], na.rm = TRUE,from=0)
  lines(density_estimate_0, col = colors[1], lwd = 2)  # lwd is line width
  
  # Density estimate for class 1
  density_estimate_1 <- density(data[data$spam_target == 1, i], na.rm = TRUE, from=0)
...
X.trans<- as.matrix(data.trans.scaled[,-ncol(data.scaled)])
...
X.train.trans <- X.trans[train_indices, ]
X.test.trans <- X.trans[-train_indices, ]
```
We may compare a full model on both the transformed and untransformed datasets.  
```{r warning=FALSE}
set.seed(123)
model <- glm(y~., data = data.frame(X.train, y=as.factor(y.train)), family = binomial)
summary(model)
model.trans <- glm(y~., data = data.frame(X.train.trans, y=as.factor(y.train)), family = binomial)
cat(paste("=============================================================\n",
          "Model fit on transformed data:\nNull deviance:",model.trans$null.deviance,
          "\nResidual deviance:", model.trans$deviance,
          "\nAIC:", model.trans$aic
          ))
```
From this output tells us many things about our fitted models. Notabaly, that the model fit on transformed data acheives a better fit than the unscaled data. This is demonstrated by the lower residual deviance and AIC values it acheives. Understnading this requires some interpration of the output, which consits of: fitted $\beta$ coefficients and respective, confidence values, as well as measures of goodness of fit: Deviance and AIC.  
...
The Akaike Information Criterion (AIC) is a specialized measure for the quality of a models fit given the number of predictive features utilized. It is defined as $AIC = 2k - 2\ln(\ell)$ 
This information, however useful, has a potential problem in that logistic regression is prone to overfitting in particular situations. This occurs when the fitting process is able to utilize insignificant information (noise) to predict the target with inflated accuracy. In order to assess our models for evidence of this, we have partitioned our datasets into training and test partitions for validation. We continue to assess how performance varies between these two partitions with an array of metrics.
```{r}
calculate_metrics <- function(predictions, ground_truths) {
  ...
  err_rate <- (confusion_matrix[1,2] + confusion_matrix[2,1]) / sum(confusion_matrix)
  return(list(
    Precision = precision,
    Recall = recall,
    F1 = f1,
...
```{r}
train.pred <- predict(model, newx = X.train, type = "response")
test.pred <- predict(model, data.frame(X.test), type = "response")
train.trans.pred <- predict(model.trans, newx = X.train.trans, type = "response")
test.trans.pred <- predict(model.trans, data.frame(X.test.trans), type = "response")

train.metrics <- calculate_metrics(train.pred, y.train)
test.metrics <- calculate_metrics(test.pred, y.test)
train.trans.metrics <- calculate_metrics(train.trans.pred, y.train)
test.trans.metrics <- calculate_metrics(test.trans.pred, y.test)

rbind(train.metrics,train.trans.metrics, test.metrics,test.trans.metrics)[,1:5]
print("Train")
train.metrics$ConfusionMatrix
print("Test")
test.metrics$ConfusionMatrix
print("Transformed Train")
train.trans.metrics$ConfusionMatrix
print("Transformed Test")
test.trans.metrics$ConfusionMatrix
```
The above metrics demonstrate:  
1. There is a small amount of overfitting, as indicated by slightly better performance in the training classes than the test classes. 
2. Transforming our predictive improved performance over both train and test sets, and leads to a model that demonstrates less overfitting than the model trained on un transformed features.  
Given these results, we will consider only the transformed dataset moving forward.
...
library(glmnet)
set.seed(123)
lasso.glmnet <- cv.glmnet(X.train.trans, y.train, alpha=1, family="binomial", type.measure="class")
lasso.train.preds <- predict(lasso.glmnet, X.train.trans, type = "response", s = lasso.glmnet$lambda.min)
lasso.test.preds <- predict(lasso.glmnet, X.test.trans, type = "response", s = lasso.glmnet$lambda.min)

lasso.train.trans.metrics <- calculate_metrics(lasso.train.preds, y.train)
lasso.test.trans.metrics <- calculate_metrics(lasso.test.preds, y.test)

rbind(lasso.train.trans.metrics,lasso.test.trans.metrics)[,1:5]
print("Train")
lasso.train.trans.metrics$ConfusionMatrix
print("Test")
lasso.test.trans.metrics$ConfusionMatrix
```
The following plot shows both the number of non-zero $\beta$ coefficients, corresponding to the number of predictor features used for a given $\log(\lambda)$ as well as the misclassification error.
```{r}
par(mfrow = c(2,1), mar = c(4, 4, 2, 2))
plot(log(lasso.glmnet$lambda), lasso.glmnet$nzero, xlab = "")
plot(lasso.glmnet, cex.main = 0.9, cex.lab = 0.9, cex.axis = 0.9)
par(mfrow = c(1,1))
```
Denoted on the plot are the points at which a choice of $\lambda$ minimizes the misclassification error and also the largest $\lambda$ choice within one standard error of the minimum. Taking this value may be useful if we are trying to minimize the number of predictor features in our model. 
```{r}
cbind(index=lasso.glmnet$index, lambda_val=lasso.glmnet$lambda[lasso.glmnet$index], num_non_zero=lasso.glmnet$nzero[lasso.glmnet$index])
```
The above table indicates that depending on our choice of the two $\lambda$, we could have a model that uses either $50$ or $34$ or our original $57$ predictor features. Furthermore, consulting the previous plot, choosing the smaller of the two models would not seem to have that bad of an impact on performance! We can calculate metrics for the smaller model to verify this is the case.
```{r}
lasso.train.preds.1se <- predict(lasso.glmnet, X.train.trans, type = "response", s = lasso.glmnet$lambda.1se)
lasso.test.preds.1se <- predict(lasso.glmnet, X.test.trans, type = "response", s = lasso.glmnet$lambda.1se)

lasso.train.trans.metrics.1se <- calculate_metrics(lasso.train.preds.1se, y.train)
lasso.test.trans.metrics.1se <- calculate_metrics(lasso.test.preds.1se, y.test)

rbind(lasso.train.trans.metrics,lasso.train.trans.metrics.1se,lasso.test.trans.metrics,lasso.test.trans.metrics.1se)[,1:4]
#print("Train")
lasso.train.trans.metrics.1se$ConfusionMatrix
#print("Test")
lasso.test.trans.metrics.1se$ConfusionMatrix
```
Here we examine which features were selected (or removed) from the model
```{r}
betas.1se <-lasso.glmnet$glmnet.fit$beta[,lasso.glmnet$index[2]]
feat.zeros <- names(betas.1se[betas.1se == 0])
feat.nzeros <- names(betas.1se[betas.1se != 0])
feat.zeros
feat.nzeros
```
Many of the predictors dropped from this smaller model where features we identified earlier as being notably colinear with other features. As part of our preprocessing, we decided not to discard any of these features, as we intended to select variables through the process of lasso regression. We can check the models work by inspecting if the features it has selected exibit less colinearity than the full model.  
In order to to do this we need to refit a glm with only the selected features. This is because we used cv.glment earlier for its ability to perform a gridsearch to select the best $\lambda$. When doing this we also passed type = "response", which reports misclassification error instead of deviance.
```{r warning=FALSE}
set.seed(123)
model.trans.reduced <- glm(y~., data = data.frame(X.train.trans[,feat.nzeros], y=as.factor(y.train)), family = binomial)
cat("=============================================================",
          "\nModel fit on transformed data:",
          "\nNull deviance:",model.trans$null.deviance,
          "\nResidual deviance:", model.trans$deviance,
...
cat("=============================================================",
          "\nReduced Model fit on transformed data:",
          "\nNull deviance:",model.trans.reduced$null.deviance,
          "\nResidual deviance:", model.trans.reduced$deviance,
          "\nAIC:", model.trans.reduced$aic
)
```
In order to assess multicolinearity in the two different sets of predictors, we compute the Variance Inflation Factor (VIF) for both. VIF is calculated for each feature by fitting a linear model with all other features as predictors and taking the $R^2$ statistic from each model:  
$$VIF_j = \frac{1}{1-R^2_{X_j|X_{-j}}}$$. Below we plot the log(VIF) values for features from each model to illustrate the difference in colinearity found in the full and reduced models:  
```{r}
library(car)
vif.full <- vif(model.trans)
vif.reduced <- vif(model.trans.reduced)
vif.df <- merge(
  data.frame(variable = names(vif.full), vif_model_trans = vif.full, stringsAsFactors = FALSE),
  data.frame(variable = names(vif.reduced), vif_model_trans_reduced = vif.reduced, stringsAsFactors = FALSE),
  by = "variable", all=TRUE
)
vif.df <- vif.df[order(vif.df$vif_model_trans, decreasing=TRUE),]
plot(vif.df$vif_model_trans, type="p",col="blue", 
     main="VIF values for features in Full and Reduced models",
     xlab="Feature (sorted by VIF)", ylab="VIF")
points(vif.df$vif_model_trans_reduced, type = 'p',  col = "red")
legend("topright", legend = c("Full", "Reduced"), col = c("blue", "red"),pch=c(19,19))
```
This plot demonstrates that for features retained in the reduced model, their VIF value will generally be smaller when fit against other retained predictors. As a result, the reduced model as chosen by lasso regresison demonstrates less colinearity among its predictors. We additionally see that for both models, though colinearity is presant, VIF values for the reduced model lie in the range of (0,5) which is generally thought to be acceptable bounds for colinearity in a dataset.

## Deviance test
As to be expected, the reduced model has increased residual variance. In general, the residual deviance of a nested model should be unable to exceed that of the full. This isn't to condemn the performance of the reduced model, though. We can see it achieves comparable AIC, and we saw that the reduced model from the glmnet method has comparable validation performance. We can more rigorously quantify this difference by performing a deviance test.  
This tests assumes the difference in residuals have a $\chi^2$ distribution and tests the following hypothesis:  
$\text{H}_0$: The reduced model provides an adequate fit.  
$\text{H}_1$: The larger models fit is significantly better.  
This can be performed with the anova method in r and reports a p-value indication support for $\text{H}_0$.
```{r}
dev.test <- anova(model.trans.reduced,model.trans,test="Chisq")
dev.test
```
This test indicate that the lower deviance achieved by the full model is statistically significant (p-value less than 0.05). However, we were quite aggressive with our removal of features to produce a more simple model. Furthermore, we deem the slight sacrifice in validation performance, though noticeable, to be justified.

## Naive Bayes
In this paper we have primarily focused on regularized logistic regression. This has been useful as it provided a means of methodically selecting varibles will fitting a simplifed model. In this section we compare our results to another methods generally reguarded as performing well at natural language processing tasks like the one we face, Naive Bayes. 
[formal]
Here we implement 4 Naive Bayes classifiers to serve as a baseline for our lasso model. In fitting we assume Poisson distribution features rather than Gaussian. As we demonstrated in our EDA, distribution of frequencies deviate quite a bit from Poisson. However, in this case interpreting frequency data as Poisson makes more sense than the alternative: Gaussian.
```{r warning=FALSE, echo=FALSE}
calculate_metrics_nb <- function(predictions, ground_truths) {
  confusion_matrix <- table(Predicted = predictions, Actual = ground_truths)
  confusion_matrix
  precision <- confusion_matrix[2, 2] / sum(confusion_matrix[2, ])
  recall <- confusion_matrix[2, 2] / sum(confusion_matrix[, 2])
  f1 <- 2 * (precision * recall) / (precision + recall)
...
library(naivebayes)
set.seed(123)
nb <- bernoulli_naive_bayes(X.train, as.factor(y.train), usepoisson = TRUE)
nb.trans <- bernoulli_naive_bayes(X.train.trans, as.factor(y.train), usepoisson = TRUE)
nb.reduced <- bernoulli_naive_bayes(X.train[,feat.nzeros], as.factor(y.train), usepoisson = TRUE)
nb.trans.reduced <- bernoulli_naive_bayes(X.train.trans[,feat.nzeros], as.factor(y.train), usepoisson = TRUE)
summary(nb)
nb.full.metrics <- calculate_metrics_nb(predict(nb, X.test, type="class"),y.test)
nb.full.trans.metrics <- calculate_metrics_nb(predict(nb.trans, X.test.trans, type="class"),y.test)
nb.reduced.metrics <- calculate_metrics_nb(predict(nb, X.test[,feat.nzeros], type="class"),y.test)
nb.reduced.trans.metrics <- calculate_metrics_nb(predict(nb.trans, X.test.trans[,feat.nzeros], type="class"),y.test)
rbind(nb.full.metrics,nb.full.trans.metrics,nb.reduced.metrics,nb.reduced.trans.metrics)[,c(1,2,3,4,5)]
```
